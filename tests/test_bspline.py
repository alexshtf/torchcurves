...
class TestBSplineFunction(unittest.TestCase):
    ...
    def test_constant_function_degree0(self):
        ...
        control_points = torch.tensor(
            [[[2.5]]], dtype=self.default_dtype, device=self.device
        )
        ...
        u_values_scalar = torch.tensor(
            [0.0, 0.5, 0.99], dtype=self.default_dtype, device=self.device
        )
        ...
        for u_val_scalar_item in u_values_scalar:
            ...
            self.assertAlmostEqual(
                points.squeeze().item(),
                control_points.squeeze().item(),
                places=5,
                msg=f"Failed for u={u.item()}",
            )
            ...
            self.assertAlmostEqual(
                u_gc.grad.squeeze().item(),
                0.0,
                places=5,
                msg=f"Grad_u non-zero for u={u.item()}",
            )
    ...
    def test_constant_function_all_cps_same(self):
        ...
        control_points = torch.full(
            (1, n_cp_c, 1), const_val, dtype=self.default_dtype, device=self.device
        )
        knots = self.generate_clamped_knot_vector(
            n_cp_c, degree, device=self.device, dtype=self.default_dtype
        )
        ...
        # u_scalar: (N,)
        u_scalar = torch.tensor(
            [0.0, 0.25, 0.5, 0.75, 1.0], dtype=self.default_dtype, device=self.device
        )
        # u: (N, M) -> (N, 1)
        ...
        expected_points = torch.full(
            (u.shape[0], 1, 1), const_val, dtype=self.default_dtype, device=self.device
        )
        ...
        torch.testing.assert_close(
            u_gc.grad, torch.zeros_like(u_gc), atol=1e-5, rtol=1e-5
        )
    ...
    def test_linear_function_degree1(self):
        ...
        control_points = torch.tensor(
            [[[0.0], [1.0]]], dtype=self.default_dtype, device=self.device
        )
        n_cp_c = control_points.shape[1]
        knots = self.generate_clamped_knot_vector(
            n_cp_c, degree, device=self.device, dtype=self.default_dtype
        ...
        u_scalar = torch.tensor(
            [-1.0, -0.5, 0.0, 0.5, 1.0], dtype=self.default_dtype, device=self.device
        )
        # u: (N,M) -> (N,1)
        u = u_scalar.unsqueeze(1)
    ...
    def test_parabola_degree2(self):
        ...
        control_points = torch.tensor(
            [[[0.0], [0.0], [1.0]]], dtype=self.default_dtype, device=self.device
        )
        n_cp_c = control_points.shape[1]
        knots = self.generate_clamped_knot_vector(
            n_cp_c, degree, device=self.device, dtype=self.default_dtype
    ...
    def test_boundary_values(self):
        ...
        control_points = torch.randn(
            1, n_cp_c, 2, dtype=self.default_dtype, device=self.device
        )
        knots = self.generate_clamped_knot_vector(
            n_cp_c, degree, device=self.device, dtype=self.default_dtype
        )
        ...
        u_start = torch.tensor(
            [[-1.0]], dtype=self.default_dtype, device=self.device
        )  # Min knot value
        u_end = torch.tensor(
            [[1.0]], dtype=self.default_dtype, device=self.device
        )  # Max knot value
        ...
        torch.testing.assert_close(
            point_start, control_points[:, 0:1, :], atol=1e-6, rtol=1e-5
        )
        torch.testing.assert_close(
            point_end, control_points[:, -1:, :], atol=1e-6, rtol=1e-5
        )
    ...
    def test_multiple_dimensions(self):
        ...
        expected_points_calc = torch.empty(
            (u_scalar.shape[0], 1, 2), dtype=self.default_dtype, device=self.device
        )
        p0, p1, p2 = (
            control_points_data[0, 0],
            control_points_data[0, 1],
            control_points_data[0, 2],
        )
    ...
    def test_batch_processing_u_values_single_curve(self):  # Renamed for clarity
        ...
        expected_points_batch = torch.empty(
            (u_batch.shape[0], 1, control_points.shape[2]),
            dtype=self.default_dtype,
            device=self.device,
        )
        ...
        torch.testing.assert_close(
            points_batch, expected_points_batch, atol=1e-6, rtol=1e-5
        )
        self.assertEqual(
            points_batch.shape, (u_batch.shape[0], 1, control_points.shape[2])
        )
        ...
        self.assertTrue(
            torch.autograd.gradcheck(
                lambda x_cp: bspline_curves(
                    u_gc_batch.detach(), x_cp, knots, degree
                ).sum(),
                cp_gc.detach().requires_grad_(True),
                eps=1e-6,
                atol=1e-5,
    ...
    def test_multiple_curves_equivalence(self):
        ...
        control_points_batched = torch.randn(
            num_curves_m, n_cp_c, dim_d, dtype=self.default_dtype, device=self.device
        )
        control_points_batched_clone_for_grad = (
            control_points_batched.clone().requires_grad_(True)
        )

        # u values for M curves: (N, M), in knot range [-1,1]
        u_batched_rand = torch.rand(
            n_samples_n, num_curves_m, dtype=self.default_dtype, device=self.device
        )
        ...
        knot_max_effective = knots[
            n_cp_c
        ]  # This is the start of the last span's p+1 knots.
        # For u, it should be knots[n_cp_c] which is the end of the domain.

        u_batched = (
            u_batched_rand * (knot_max_effective - knot_min_effective)
            + knot_min_effective
        )
        u_batched_clone_for_grad = u_batched.clone().requires_grad_(True)

        # 1. Evaluate all curves together
        points_batched = bspline_curves(
            u_batched_clone_for_grad,
            control_points_batched_clone_for_grad,
            knots,
            degree,
        )
        ...
        for i in range(num_curves_m):
            cp_single_grad_target = (
                control_points_batched[i : i + 1, :, :]
                .detach()
                .clone()
                .requires_grad_(True)
            )
            u_single_grad_target = (
                u_batched[:, i : i + 1].detach().clone().requires_grad_(True)
            )

            points_single_eval = bspline_curves(
                u_single_grad_target, cp_single_grad_target, knots, degree
            )
            ...
            expected_grad_cp_from_individuals[i : i + 1, :, :] = (
                cp_single_grad_target.grad
            )

        torch.testing.assert_close(
            grad_u_batched_actual,
            expected_grad_u_from_individuals,
            atol=1e-6,
            rtol=1e-5,
        )
        torch.testing.assert_close(
            grad_cp_batched_actual,
            expected_grad_cp_from_individuals,
            atol=1e-6,
            rtol=1e-5,
        )
...
class TestBSplineCurveModule(unittest.TestCase):
    ...
    def test_init_with_int(self):
        ...
        self.assertEqual(
            module.control_points.shape, (num_curves, n_cps_per_curve, dim)
        )
    ...
    def test_init_with_tensor(self):
        ...
        knots_tensor = torch.tensor(
            [0.0, 0.0, 0.0, 0.5, 1.0, 1.0, 1.0], dtype=self.default_dtype
        )
        expected_n_cps_per_curve = 4  # 7 - 2 - 1 = 4

        module = (
            BSplineCurve(
                num_curves=num_curves, dim=dim, degree=degree, knots_config=knots_tensor
            )
            .to(self.device)
            .to(self.default_dtype)
        )
        ...
        self.assertEqual(
            module.control_points.shape, (num_curves, expected_n_cps_per_curve, dim)
        )
        torch.testing.assert_close(
            module.knots, knots_tensor.to(self.device).to(self.default_dtype)
        )

    def test_init_errors(self):
        with self.assertRaisesRegex(ValueError, "must be greater than the degree"):
            BSplineCurve(
                num_curves=1, dim=2, degree=3, knots_config=3
            )  # n_cp <= degree
    ...
    def test_forward_pass_shape_and_device(self):
        ...
        # u: (N, M)
        u_scalar = torch.linspace(
            -1, 1, batch_size, device=self.device, dtype=self.default_dtype
        )
    ...
    def test_backward_pass(self):
        ...
        # u: (N,M)
        u = torch.tensor(
            [[-0.7], [0.6]], device=self.device, dtype=self.default_dtype
        )  # N=2, M=1
        ...
        self.assertEqual(
            module.control_points.grad.shape, module.control_points.shape
        )  # (1,C,D)
    ...
    def test_gradcheck_module(self):
        ...
        # u_gc: (N,M)
        u_gc = torch.tensor(
            [[-0.75], [0.25]], device=self.device, dtype=self.default_dtype
        ).requires_grad_(True)
        ...
        self.assertTrue(
            torch.autograd.gradcheck(
                lambda u_in, cp_in: bspline_curves(
                    u_in, cp_in, knots, current_degree
                ).sum(),
                (u_gc, cp_gc),
                eps=1e-6,
                atol=1e-4,
        ...
        u_gc_mod = torch.tensor(
            [[-0.6]], device=self.device, dtype=self.default_dtype
        ).requires_grad_(
            True
        )  # (1,1)
        ...
        self.assertTrue(
            torch.autograd.gradcheck(
                lambda u_in: module_clone(u_in).sum(),
                u_gc_mod,
                eps=1e-6,
                atol=1e-4,
                rtol=1e-3,
    ...
    def test_device_movement(self):
        ...
        u_cuda = torch.tensor(
            [[-0.7], [0.6]], device="cuda", dtype=self.default_dtype
        )  # (2,1)
