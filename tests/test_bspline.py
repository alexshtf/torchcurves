...
class TestBSplineFunction(unittest.TestCase):
    ...
    def test_linear_function_degree1(self):
        ...
        self.assertTrue(
            torch.autograd.gradcheck(
                lambda x: BSplineFunction.apply(x, cp_gc.detach(), knots, degree).sum(),
        ...
        self.assertTrue(
            torch.autograd.gradcheck(
                lambda x: BSplineFunction.apply(u_gc.detach(), x, knots, degree).sum(),
    ...
    def test_parabola_degree2(self):
        ...
        self.assertTrue(
            torch.autograd.gradcheck(
                lambda x_cp: BSplineFunction.apply(u_gc.detach(), x_cp, knots, degree).sum(),
                cp_gc.detach().requires_grad_(True),
                eps=1e-6,
                atol=1e-5,
                rtol=1e-3,
    ...
    def test_boundary_values(self):
        ...
        point_start = BSplineFunction.apply(u_start, control_points, knots, degree)  # (1,1,2)
        point_end = BSplineFunction.apply(u_end, control_points, knots, degree)  # (1,1,2)
    ...
    def test_multiple_dimensions(self):
        ...
        points = BSplineFunction.apply(u, control_points_data, knots, degree)
    ...
    def test_multiple_curves_equivalence(self):
        ...
        points_batched = BSplineFunction.apply(
        ...
        for i in range(num_curves_m):
            ...
            points_single = BSplineFunction.apply(u_single, cp_single, knots, degree)  # Output (N, 1, D)
        ...
        for i in range(num_curves_m):
            ...
            points_single_eval = BSplineFunction.apply(u_single_grad_target, cp_single_grad_target, knots, degree)
...
class TestBSplineCurveModule(unittest.TestCase):
    ...
    def test_gradcheck_module(self):
        ...
        self.assertTrue(
            torch.autograd.gradcheck(
                lambda u_in, cp_in: BSplineFunction.apply(u_in, cp_in, knots, current_degree).sum(),
                (u_gc, cp_gc),
                eps=1e-6,
                atol=1e-4,
                rtol=1e-3,
